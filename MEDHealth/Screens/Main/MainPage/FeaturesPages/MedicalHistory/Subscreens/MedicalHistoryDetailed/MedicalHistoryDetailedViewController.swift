//
//  MedicalHistoryDetailedViewController.swift
//  MEDCheck
//
...
final class MedicalHistoryDetailedViewController: UIViewController {
    ...
    private lazy var takingDescriptionLabel: UILabel = {
        let label = UILabel()
        label.font = UIFont(name: "RalewayRoman-Bold", size: 24)
        label.textColor = .black
        label.numberOfLines = 0
        label.textAlignment = .center
        return label
    ...
    // MARK: - Public methods
    public func setup(with note: MedicalHistory) {
        takingTitleLabel.text  = note.date
        takingDescriptionLabel.text = note.content
    }
